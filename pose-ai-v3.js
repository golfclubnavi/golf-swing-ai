...
/* 0.15 = very smooth / 0.35 = more responsive */
...
function smoothLandmarks(lm) {
  ...
  smoothed = lm.map((p, i) => {
    ...
    // Low-confidence points move much less, reducing sudden jumps.
    const a = vis >= MIN_VISIBILITY ? SMOOTH_ALPHA : 0.06;

    return {
      ...p,
      x: old.x + (p.x - old.x) * a,
      y: old.y + (p.y - old.y) * a,
      z: old.z + ((p.z ?? 0) - (old.z ?? 0)) * a,
      visibility: vis
    };
  });
...
/*
  Computes the actual visible video rectangle inside an object-fit:contain box.
  This fixes the common overlay offset when portrait/landscape video leaves black bars.
*/
