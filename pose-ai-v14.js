...
function robustSmooth(lm) {
  ...
  return lm.map((p,i)=>{
    ...
    let alpha;
    if ((p.visibility ?? 1) < MIN_VISIBILITY) alpha = 0.06;
    else if (rawMove < 0.0025) alpha = 0.18;  // still: keep jitter suppression
    else if (rawMove < 0.008) alpha = 0.42;   // small motion
    else if (rawMove < 0.020) alpha = 0.72;   // normal swing motion
    else alpha = 0.94;                        // fast swing: near-immediate tracking
