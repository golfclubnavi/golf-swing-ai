...
function robustSmooth(lm) {
  ...
  return lm.map((p,i)=>{
    ...
    let alpha;
    if ((p.visibility ?? 1) < MIN_VISIBILITY) alpha = 0.05;
    else if (rawMove < 0.003) alpha = 0.16;   // still: suppress jitter
    else if (rawMove < 0.010) alpha = 0.32;   // small motion
    else if (rawMove < 0.025) alpha = 0.58;   // normal swing motion
    else alpha = 0.82;                        // fast swing: prioritize tracking
